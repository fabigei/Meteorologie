...
int main(int argc, char** args){
    ...
    // Verschiede Optical Thicknesses verwenden
    //double beta0=0.001;
	...
	for(beta0=1e-6;beta0<0.002;beta0*=1.2){
		Edn.resize(0);
		Eup.resize(0); 	
		for(int lambdaBand=0;lambdaBand<lambda_vec.size()-1;lambdaBand++){
				...
				for (int i=0;i<Nlayer;i++){
					...
					//cout<<"sssssss"<<endl;
					//print_vec(bb_vec);
				rt(Nlayer, Nmu, beta0, lambda_vec[lambdaBand], lambda_vec[lambdaBand+1]-1,he_vec, te_vec,  opt_thick_vec, bb_vec, Edn, Eup );
				}
				
			cout<<beta0<<"  "<< Eup[0]<<endl;
		}
}
